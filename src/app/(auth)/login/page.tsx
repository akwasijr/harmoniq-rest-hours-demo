...
export function LoginForm() {
  ...
  const handleMagicLink = async (e: React.FormEvent) => {
    ...
    if (IS_MOCK_MODE) {
      setError("Magic links are disabled in demo mode. Use a demo account with password 'demo123'.");
      return;
    }
    try {
      setIsLoading(true);
  ...
  const handleForgotPassword = async () => {
    ...
    if (IS_MOCK_MODE) {
      setError("");
      setSuccess("Password reset is disabled in demo mode. Sign in with password 'demo123'.");
      return;
    }
    try {
      setIsLoading(true);
