...
import { IS_MOCK_MODE, mockLogin } from "@/hooks/use-auth";
...
export default function AdminLoginPage() {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (IS_MOCK_MODE) {
        if (password !== "demo123") {
          setPasswordError("Invalid password. Use 'demo123' for demo mode.");
          setIsLoading(false);
          return;
        }
        ...
        if (!mockUser) {
          setError("No demo admin account found with that email.");
          setIsLoading(false);
          return;
        }

        if (mockUser.role !== "super_admin" && mockUser.role !== "company_admin") {
        ...
        if (mockUser.role === "super_admin") {
          ...
          window.location.href = buildCompanyDestination(DEFAULT_COMPANY_SLUG, "dashboard");
        }
        return;
      }
