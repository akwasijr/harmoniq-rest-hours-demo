...
import { getSupabasePublishableKey, getSupabaseUrl, isMockModeEnabled } from "@/lib/supabase/public-env";
...
export function validateEnv() {
  ...
  if (env.missingRequired.length > 0) {
    if (isMockMode) {
      console.warn("[ENV] Running in demo mode without Supabase. Auth and persistence use local mock data.");
    } else {
  ...
  if (!env.hasSupabaseAdminKey && !isMockMode) {
