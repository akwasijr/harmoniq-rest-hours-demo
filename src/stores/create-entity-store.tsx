...
export function createEntityStore<T extends IdEntity>(
  ...
  function Provider({ children }: { children: React.ReactNode }) {
    const [items, setItems] = React.useState<T[]>(() => {
      ...
      // If cached items are missing fields that exist in initial data, reset cache
      // (handles schema migrations in mock mode)
      if (initialData.length > 0 && cached.length > 0) {
        const initialKeys = Object.keys(initialData[0] as object);
        const cachedKeys = new Set(Object.keys(cached[0] as object));
        const missingKeys = initialKeys.filter((k) => !cachedKeys.has(k));
