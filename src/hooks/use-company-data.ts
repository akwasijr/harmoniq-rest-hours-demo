...
import { useRestHoursRecordsStore } from "@/stores/rest-hours-store";
import { useVesselsStore } from "@/stores/vessels-store";
import { useVoyagesStore } from "@/stores/voyages-store";
import { useCrewAssignmentsStore } from "@/stores/crew-assignments-store";
...
export function useCompanyData() {
  ...
  const restHoursRecordsStore = useRestHoursRecordsStore();
  const vesselsStore = useVesselsStore();
  const voyagesStore = useVoyagesStore();
  const crewAssignmentsStore = useCrewAssignmentsStore();
  ...
  const restHoursRecords = useMemo(() => restHoursRecordsStore.itemsForCompany(companyId), [restHoursRecordsStore, companyId]);
  const vessels = useMemo(() => vesselsStore.itemsForCompany(companyId), [vesselsStore, companyId]);
  const voyages = useMemo(() => voyagesStore.itemsForCompany(companyId), [voyagesStore, companyId]);
  const crewAssignments = useMemo(() => crewAssignmentsStore.itemsForCompany(companyId), [crewAssignmentsStore, companyId]);
  ...
  return {
    ...
    stores: {
      ...
      restHoursRecords: restHoursRecordsStore,
      vessels: vesselsStore,
      voyages: voyagesStore,
      crewAssignments: crewAssignmentsStore,
    },
    companyId,
  };
}
