...
// ============================================
// MARITIME — VESSEL & VOYAGE
// ============================================

export type VesselType = "bulk_carrier" | "container" | "tanker" | "general_cargo" | "offshore" | "passenger" | "other";
...
export interface Voyage {
  ...
  vessel_id: string;
  company_id: string;
  name: string; // e.g. "Voyage 2025-03 Rotterdam–Bergen"
  departure_port: string;
  destination_port: string | null;
  embarkation_date: string; // ISO date
  expected_end_date: string; // ISO date
  actual_end_date: string | null;
  status: VoyageStatus;
  master_id: string; // captain user ID
  created_at: string;
  updated_at: string;
}

export type CrewAssignmentStatus = "active" | "signed_off";
...
export interface VoyageCrewAssignment {
  ...
  company_id: string;
  user_id: string;
  role_on_board: string; // Master, Chief Officer, AB, etc.
  department: string;    // Deck, Engine, Galley
  is_watchkeeper: boolean;
  board_date: string;    // ISO date
...
export type RestHoursRecordStatus = "draft" | "submitted" | "approved" | "rejected" | "forwarded_to_hr";

/** A single rest period within a day (max 2 per day per MLC rules) */
export interface RestPeriod {
  from: string; // HH:mm
  to: string;   // HH:mm
}

/** One day's entry in the rest-hours record */
export interface RestHoursDayEntry {
  date: string; // ISO date YYYY-MM-DD
  is_at_sea: boolean;
  rest_periods: RestPeriod[];
  work_periods: RestPeriod[];
  total_rest_hours: number;
  total_work_hours: number;
  comments: string;
}

/** Compliance check result for a single day */
export interface RestHoursComplianceCheck {
  date: string;
  min_rest_24h_ok: boolean;    // >= 10h in any 24h period
  min_continuous_ok: boolean;  // at least one rest period >= 6h
  total_rest_hours: number;
}

/** Rolling 7-day compliance check */
export interface RestHours7DayCheck {
  period_end: string;
  total_rest_hours: number;
  min_77h_ok: boolean; // >= 77h rest in any 7-day period
}

/** A monthly rest-hours record for a seafarer */
export interface RestHoursRecord {
  id: string;
  company_id: string;
  // Vessel/voyage references
  vessel_id: string;
  voyage_id: string;
  // Denormalized vessel info (for display/export)
  vessel_name: string;
  imo_number: string;
  flag_state: string;
  // Seafarer info
  seafarer_id: string;      // user ID
  ...
  year: number;
  // Data
  entries: RestHoursDayEntry[];
  // Compliance summary (computed on submission)
  daily_compliance: RestHoursComplianceCheck[];
  weekly_compliance: RestHours7DayCheck[];
  has_violations: boolean;
  // Workflow — dual signature
  status: RestHoursRecordStatus;
  seafarer_signed_at: string | null;  // seafarer confirms accuracy
  submitted_at: string | null;
  master_signed_at: string | null;    // captain countersigns (approval)
  reviewed_by: string | null;         // captain/master user ID
  reviewed_at: string | null;
  reviewer_notes: string;
  violation_comment: string;          // seafarer's explanation for violations
  forwarded_to_hr_at: string | null;  // when record was sent to HR
  // Late entry warning
  is_late_entry: boolean; // true if submitted > 48h after period end
  // Timestamps
  created_at: string;
  updated_at: string;
}
